...
export class BacktestingComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    console.log('BacktestingComponent: Constructor started');
    try {
      this.configForm = this.createForm();
      console.log('BacktestingComponent: Form created successfully');
    } catch (error) {
      console.error('BacktestingComponent: Form creation failed:', error);
      throw error;
    }
    console.log('BacktestingComponent: Constructor completed');
  }

  ngOnInit(): void {
    console.log('BacktestingComponent: ngOnInit started');
    
    try {
      console.log('BacktestingComponent: Setting up default dates');
      // Set default dates first (most critical)
      const endDate = new Date();
      const startDate = new Date();
      startDate.setFullYear(startDate.getFullYear() - 2); // Default to 2 years
      
      console.log('BacktestingComponent: Patching form with dates:', { startDate, endDate });
      this.configForm.patchValue({
        startDate,
        endDate
      });
      console.log('BacktestingComponent: Form patched successfully');

      console.log('BacktestingComponent: Setting up excluded coins watcher');
      // Watch excluded coins text changes
      this.configForm.get('excludedCoinsText')?.valueChanges
        .pipe(takeUntil(this.destroy$))
        .subscribe(value => {
          console.log('BacktestingComponent: Excluded coins changed:', value);
          this.updateExcludedCoins(value);
        });
      console.log('BacktestingComponent: Excluded coins watcher setup complete');

      // Prefetch common coin data (make this completely optional)
      console.log('BacktestingComponent: Attempting to prefetch common coins (optional)');
      try {
        if (this.historicalDataService && typeof this.historicalDataService.prefetchCommonCoins === 'function') {
          ...
          console.log('BacktestingComponent: Prefetch started successfully');
        } else {
      ...
      console.log('BacktestingComponent: ngOnInit completed successfully');
    } catch (error) {
      ...
      console.log('BacktestingComponent: Error state set, component should still render');
    }
  }
